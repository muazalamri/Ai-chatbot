...
async def main(text):
    a = Asseten()
    d= await a.answer(text)
    return d


from flask import Flask,request,render_template
from styling import md_to_html
app=Flask(__name__)
...
def index():
    ...
    if request.method=='POST':
        ...
        d=asyncio.run(main(prompt))
        d=md_to_html(str(d['answer'])).replace('\n',"<br>").replace('<pre>','<p>')
    return render_template('index.html').replace('{text}',str
